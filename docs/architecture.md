...
- **`forge/converter.py`** — Converts each validated rule into per-platform queries via pySigma and its backend plugins, writing them under `dist/<platform>/`: Splunk SPL, Sentinel KQL (via the pySigma Microsoft 365 Defender backend; usable in Microsoft Sentinel / Defender XDR advanced hunting), Elastic NDJSON, and a Wazuh query (Elastic/OpenSearch-compatible queries for the Wazuh indexer — its indexer is OpenSearch, so the Lucene query applies directly; a native Wazuh XML backend is on the roadmap). pySigma and its backends are imported defensively, so the module is always import-safe; if a backend is missing it is simply skipped, and `build_all` creates each backend's output directory lazily so a backend that yields no output leaves no empty directory. A single failing rule is logged and skipped rather than aborting the whole backend.
...
- **`forge/cli.py`** — The command-line entry point (`forge build`, `forge test`, `forge coverage`, also runnable as `python -m forge.cli ...`). `build` loads all rules once and dispatches to the converter; `coverage` regenerates the ATT&CK layer; `test` shells out to `pytest` to run the detection suite. Built on `argparse`.

## Output layout

The `dist/` tree is intentionally **bifurcated by platform** — `dist/splunk/` (SPL), `dist/sentinel/` (KQL), `dist/elastic/` (NDJSON), `dist/wazuh/` (Elastic/OpenSearch-compatible queries for the Wazuh indexer; a native Wazuh XML backend is on the roadmap) — so each consuming team takes only the format it uses, plus the shared `dist/attack-navigator-layer.json` coverage map.
